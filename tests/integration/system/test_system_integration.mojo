...
fn main():
    ...
    try:
        ...
        total_tests = 0
        passed_tests = 0
        real_time_target_ms = 40.0  # 25 Hz = 40ms cycle time
    ...
    try:
        ...
        if has_nvidia or has_amd:
            ...
            matrix_buffer = ctx.enqueue_create_buffer[DType.float64](matrix_size * matrix_size)
        ...
        real_time_target_ms = 40.0
        real_time_performance = execution_time_ms < real_time_target_ms
        performance_score = min(100.0, (real_time_target_ms / execution_time_ms) * 100.0)
    ...
    try:
        ...
        for cycle in range(control_cycles):
            ...
            if has_nvidia or has_amd:
                ...
                state_buffer = ctx.enqueue_create_buffer[DType.float64](4)  # [pos, vel, angle, cmd]
                ...
                control_buffer = ctx.enqueue_create_buffer[DType.float64](1)
        ...
        real_time_performance = cycle_success_rate > 95.0  # 95% of cycles must be real-time
        performance_score = cycle_success_rate
    ...
    try:
        ...
        for test in range(stability_tests):
            ...
            if has_nvidia or has_amd:
                ...
                for i in range(min(500, 250)):  # Reduced for stability testing
                    stability_value = Float64(test * 500 + i) * 0.0001
        ...
        real_time_performance = stability_rate > 90.0  # 90% stability required
        performance_score = stability_rate
        cpu_fallback_tested = True
    ...
    try:
        ...
        gpu_available = has_nvidia or has_amd
        total_tests = 4  # Matrix, Neural, Control, Stability
