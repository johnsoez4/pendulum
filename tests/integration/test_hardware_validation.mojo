...
fn main():
    ...
    print(
        "Testing hardware acceleration validation with real MAX Engine"
        " DeviceContext API"
    )
    ...
    try:
        ...
        for i in range(len(operations)):
            ...
            if has_nvidia or has_amd:
                ...
                print(
                    "    ⚠️  GPU execution validation skipped -", operation_name
                )
        ...
        if total_operations > 0:
            success_rate = (
                Float64(successful_operations)
                / Float64(total_operations)
                * 100.0
            )
    ...
    try:
        ...
        for i in range(len(memory_test_sizes)):
            ...
            if has_nvidia or has_amd:
                ...
                print(
                    "    ⚠️  GPU memory validation skipped - no GPU available"
                )
        ...
        if expected_memory > 0.0:
            memory_efficiency = min(
                100.0, (total_allocated_memory / expected_memory) * 100.0
            )
    ...
    try:
        ...
        for i in range(len(perf_operations)):
            ...
            if has_nvidia or has_amd:
                ...
                print(
                    "      - Memory bandwidth:", memory_bandwidth_gb_s, "GB/s"
                )
                print("      - Data size:", data_size, "elements")
            else:
                print(
                    "    ⚠️  GPU performance monitoring skipped -",
                    operation_name,
                )

        # Calculate average performance metrics
        if performance_samples > 0:
            avg_execution_time = total_execution_time_ms / Float64(
                performance_samples
            )
            avg_memory_bandwidth = total_memory_bandwidth_gb_s / Float64(
                performance_samples
            )
            ...
            print(
                "    - Average memory bandwidth:", avg_memory_bandwidth, "GB/s"
            )
