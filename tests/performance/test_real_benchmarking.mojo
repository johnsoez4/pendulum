...
fn main():
    ...
    print(
        "Testing real GPU vs CPU performance benchmarking with MAX Engine"
        " DeviceContext API"
    )
    ...
    try:
        ...
        for i in range(iterations):
            ...
            for row in range(len(matrix_a)):
                ...
                for col in range(len(matrix_b[0]) if len(matrix_b) > 0 else 0):
                    var sum = 0.0
                    for k in range(
                        len(matrix_a[0]) if len(matrix_a) > 0 else 0
                    ):
        ...
        if has_nvidia or has_amd:
            print(
                "  REAL GPU: Running matrix operations with DeviceContext"
                " timing..."
            )
            ...
            for i in range(iterations):
                ...
                buffer_a = ctx.enqueue_create_buffer[DType.float64](
                    min(buffer_size, 1024)
                )
                buffer_b = ctx.enqueue_create_buffer[DType.float64](
                    min(buffer_size, 1024)
                )
                var buffer_result = ctx.enqueue_create_buffer[DType.float64](
                    min(buffer_size, 1024)
                )
            ...
            cpu_throughput = (
                Float64(iterations) * ops_per_iteration / (cpu_time_ms / 1000.0)
            )
            gpu_throughput = (
                Float64(iterations) * ops_per_iteration / (gpu_time_ms / 1000.0)
            )
            memory_usage_mb = Float64(matrix_size * matrix_size * 8 * 3) / (
                1024.0 * 1024.0
            )
    ...
    try:
        ...
        if has_nvidia or has_amd:
            print(
                "  REAL GPU: Running neural network inference with"
                " DeviceContext..."
            )
            ...
            nn_memory_usage_mb = Float64(batch_size * input_dim * 8) / (
                1024.0 * 1024.0
            )
    ...
    try:
        ...
        for i in range(len(memory_sizes)):
            ...
            if has_nvidia or has_amd:
                ...
                gpu_buffer = ctx.enqueue_create_buffer[DType.float64](
                    memory_size
                )
