    ...
    if has_nvidia or has_amd:
        try:
            ...
            for size in sizes:
                ...
                # Create GPU buffers
                a_buffer = device_context.enqueue_create_buffer[DType.float64](buffer_size)
                b_buffer = device_context.enqueue_create_buffer[DType.float64](buffer_size)
                c_buffer = device_context.enqueue_create_buffer[DType.float64](buffer_size)
                ...
                device_context.enqueue_function[gpu_matrix_multiply_benchmark_kernel](
                    c_buffer.unsafe_ptr(),
                    a_buffer.unsafe_ptr(),
                    b_buffer.unsafe_ptr(),
                    size,
    ...
    if has_nvidia or has_amd:
        try:
            ...
            for config in configs:
                ...
                # Create GPU buffers
                input_buffer = device_context.enqueue_create_buffer[DType.float64](input_size)
                weights_buffer = device_context.enqueue_create_buffer[DType.float64](input_size * output_size)
                biases_buffer = device_context.enqueue_create_buffer[DType.float64](output_size)
                output_buffer = device_context.enqueue_create_buffer[DType.float64](output_size)
                ...
                for i in range(output_size):
                    row = List[Float64]()
                    for j in range(input_size):
                        row.append(Float64((i * input_size + j) * 7 % 100) / 100.0)
