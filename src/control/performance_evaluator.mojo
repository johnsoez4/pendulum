...
struct PerformanceMetrics(Copyable, Movable):
    ...
    fn meets_targets(self) -> Bool:
        ...
        return (
            self.inversion_success_rate >= TARGET_SUCCESS_RATE
            and self.average_stability_time >= MIN_STABILITY_DURATION
            and self.safety_compliance >= 0.95
        )
    ...
    fn get_overall_score(self) -> Float64:
        ...
        score = (
            weights[0]
            * min(1.0, self.inversion_success_rate / TARGET_SUCCESS_RATE)
            + weights[1]
            * min(1.0, self.average_stability_time / MIN_STABILITY_DURATION)
...
struct PerformanceEvaluator:
    ...
    fn initialize_evaluator(mut self) -> Bool:
        ...
        print(
            "Performance Evaluator initialized with",
            len(self.evaluation_scenarios),
            "scenarios",
        )
        return True

    fn evaluate_control_performance(
        mut self, trained_parameters: ParameterSet
    ) -> PerformanceMetrics:
        """
        ...
        for i in range(len(self.evaluation_scenarios)):
            ...
            var scenario_results = self._evaluate_scenario(
                scenario, trained_parameters
            )
        ...
        safety_compliance = Float64(
            total_evaluations - safety_violations
        ) / Float64(total_evaluations)

        # Additional performance metrics
        robustness_score = self._calculate_robustness_score(trained_parameters)
        real_time_score = self._evaluate_real_time_performance(
            trained_parameters
        )
    ...
    fn _evaluate_scenario(
        self, scenario: EvaluationScenario, parameters: ParameterSet
        ...
        for i in range(len(scenario.initial_states)):
            ...
            var result = self._simulate_control_performance(
                initial_state, parameters, scenario.evaluation_duration
            )
        ...
        safety_score = (
            Float64(len(scenario.initial_states) - safety_violations)
            / num_states
        )

        return (avg_success, avg_stability, avg_effort, avg_error, safety_score)

    fn _simulate_control_performance(
        self,
        ...
        control_effort = (
            parameters.mpc_weight_control * 20.0 + abs(initial_angle) * 0.1
        )
    ...
    fn _calculate_robustness_score(self, parameters: ParameterSet) -> Float64:
        ...
        for i in range(robustness_tests):
            ...
            variation = 0.8 + 0.4 * Float64(i) / Float64(
                robustness_tests
            )  # 80% to 120%
    ...
    fn _print_performance_summary(self):
        ...
        print(
            ...
            self.current_metrics.inversion_success_rate * 100.0,
            "%",
        )
        print(
            "  Average Stability Time:",
            self.current_metrics.average_stability_time,
            "seconds",
        )
        print(
            "  Maximum Stability Time:",
            self.current_metrics.maximum_stability_time,
            "seconds",
        )
        print(
            "  Average Settling Time:",
            self.current_metrics.average_settling_time,
            "seconds",
        )
        print(
            "  Control Effort (RMS):",
            self.current_metrics.control_effort_rms,
            "V",
        )
        print(
            "  Tracking Error (RMS):",
            self.current_metrics.tracking_error_rms,
            "°",
        )
        print(
            "  Robustness Score:",
            self.current_metrics.robustness_score * 100.0,
            "%",
        )
        print(
            "  Real-time Performance:",
            self.current_metrics.real_time_performance * 100.0,
            "%",
        )
        print(
            "  Safety Compliance:",
            self.current_metrics.safety_compliance * 100.0,
            "%",
        )
        print(
            "  Energy Efficiency:",
            self.current_metrics.energy_efficiency * 100.0,
            "%",
        )
        print(
            "  Overall Score:",
            self.current_metrics.get_overall_score() * 100.0,
            "%",
        )
        print()
        ...
        print(
            ...
            "✓" if self.current_metrics.inversion_success_rate >= 0.70 else "✗",
        )
        print(
            "  Stability Time Target (>15s):",
            "✓" if self.current_metrics.average_stability_time >= 15.0 else "✗",
        )
        print(
            "  Safety Compliance (>95%):",
            "✓" if self.current_metrics.safety_compliance >= 0.95 else "✗",
        )
        print(
            "  Overall Target Achievement:",
            "✓" if self.current_metrics.meets_targets() else "✗",
        )
        print()
        ...
        if self.current_metrics.meets_targets():
            ...
            print(
                "⚠ Some performance targets not met - further optimization"
                " needed"
            )
