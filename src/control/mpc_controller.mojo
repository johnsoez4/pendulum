...
struct MPCPrediction(Copyable, Movable):
    ...
    fn is_valid(self) -> Bool:
        ...
        return (
            ...
            and len(self.predicted_states) == MPC_PREDICTION_HORIZON
            and len(self.control_sequence) == MPC_CONTROL_HORIZON
        )
...
struct MPCObjective(Copyable, Movable):
    ...
    fn compute_stage_cost(
        self, state: List[Float64], control: Float64, prev_control: Float64
    ) -> Float64:
        ...
        return (
            self.weight_angle * angle_error * angle_error
...
struct MPCController:
    ...
    fn compute_mpc_control(
        ...
        if (
            mpc_prediction.is_valid()
            and len(mpc_prediction.control_sequence) > 0
        ):
            optimal_control = mpc_prediction.control_sequence[0]

        # Apply safety constraints
        optimal_control = self._apply_mpc_constraints(
            optimal_control, current_state
        )
    ...
    fn _solve_mpc_optimization(
        ...
        for iteration in range(MPC_MAX_ITERATIONS):
            ...
            total_cost = self._evaluate_trajectory_cost(
                prediction.predicted_states, control_sequence
            )
            ...
            improved_sequence = self._gradient_descent_step(
                current_state, control_sequence
            )
        ...
        final_prediction = self._predict_trajectory(
            current_state, best_control_sequence
        )
    ...
    fn _predict_trajectory(
        self, initial_state: List[Float64], control_sequence: List[Float64]
    ) -> MPCPrediction:
        ...
        for i in range(MPC_PREDICTION_HORIZON):
            ...
            if i < len(control_sequence):
                ...
                control_input = control_sequence[
                    len(control_sequence) - 1
                ]  # Hold last control
            ...
            # Compute stage cost
            stage_cost = self.mpc_objective.compute_stage_cost(
                next_state, control_input, prev_control
            )
            cost_trajectory.append(stage_cost)

            # Check constraints
            violates_constraints = self._check_state_constraints(
                next_state, control_input
            )
    ...
    fn _evaluate_trajectory_cost(
        ...
        for i in range(min(len(predicted_states) - 1, len(control_sequence))):
            ...
            var stage_cost = self.mpc_objective.compute_stage_cost(
                state, control, prev_control
            )
    ...
    fn _gradient_descent_step(
        self, current_state: List[Float64], control_sequence: List[Float64]
    ) -> List[Float64]:
        ...
        for i in range(len(control_sequence)):
            ...
            cost_plus = self._evaluate_trajectory_cost(
                self._predict_trajectory(
                    current_state, perturbed_sequence_plus
                ).predicted_states,
            ...
            cost_minus = self._evaluate_trajectory_cost(
                self._predict_trajectory(
                    current_state, perturbed_sequence_minus
                ).predicted_states,
    ...
    fn _check_state_constraints(
        self, state: List[Float64], control: Float64
    ) -> Bool:
    ...
    fn _apply_mpc_constraints(
        self, control: Float64, current_state: List[Float64]
    ) -> Float64:
    ...
    fn get_mpc_performance(self) -> (Float64, Float64, Int, Float64):
        ...
        var convergence_rate = (
            Float64(converged_count)
            / Float64(len(self.prediction_history))
            * 100.0
        )
