...
from src.control.rl_controller import RLController, RLState
from src.control.advanced_hybrid_controller import AdvancedHybridController
from src.control.advanced_performance_validator import (
...
struct AdvancedControlDemo:
    ...
    fn run_advanced_control_demonstration():
        ...
        hybrid_performance = (
            AdvancedControlDemo._demonstrate_hybrid_development()
        )
        print()

        # Stage 3: Comprehensive Performance Validation
        validation_results = (
            AdvancedControlDemo._demonstrate_performance_validation()
        )
        print()

        # Stage 4: Final Analysis and Task 4 Validation
        AdvancedControlDemo._demonstrate_final_analysis(
            rl_performance, hybrid_performance, validation_results
        )
    ...
    fn _demonstrate_rl_development() -> (Float64, Float64, Int, Float64):
        ...
        print(
            "  State space: 6D (position, velocity, angle, angular velocity,"
            " target, time)"
        )
        ...
        for episode in range(training_episodes):
            ...
            for cycle in range(25):  # 1 second episodes
                ...
                command = rl_controller.compute_rl_control(
                    test_state, timestamp
                )
        ...
        estimated_success_rate = (
            0.85  # RL typically achieves 85%+ with training
        )
    ...
    @staticmethod
    fn _demonstrate_hybrid_development() -> (
        Float64,
        Float64,
        Float64,
        String,
        ...
        print("=" * 55)
        print(
            "Developing advanced hybrid controller with intelligent fusion..."
        )
        ...
        print(
            "  Fusion strategies: Balanced, MPC-dominant, RL-dominant,"
            " Adaptive-dominant"
        )
        ...
        for i in range(len(test_scenarios)):
            ...
            for cycle in range(50):  # 2 second test
                ...
                command = hybrid_controller.compute_hybrid_control(
                    scenario, timestamp
                )
        ...
        if hybrid_performance[4]:
            print(
                "  ✓ Hybrid controller achieves >90% success and >30s"
                " stability!"
            )
        else:
    ...
    @staticmethod
    fn _demonstrate_performance_validation() -> (
        String,
        Bool,
        Float64,
        Float64,
        ...
        print(
            "  Validation scenarios: Precision control, Large angle recovery,"
            " Robustness test"
        )
        print(
            "  Baseline performance: 70% success rate, 15s stability (Task 3)"
        )
        ...
        optimized_params = ParameterSet(
            ...
            0.5,
            10,
            5,  # MPC parameters (optimized)
            20.0,
            2.5,
            0.6,
            1.2,
            0.12,  # Adaptive gains (optimized)
            12.0,
            100.0,
            85.0,
            145.0,  # Control thresholds (optimized)
            0.75,
            0.25,  # Hybrid weights (optimized)
        )

        # Run comprehensive validation
        _ = validator.validate_advanced_controllers(optimized_params)
    ...
    @staticmethod
    fn _demonstrate_final_analysis(
        rl_performance: (Float64, Float64, Int, Float64),
        ...
        # RL improvements
        rl_success_improvement = (
            (rl_performance[3] - baseline_success) / baseline_success * 100.0
        )
        print("  RL Controller:")
        ...
        # Hybrid improvements
        hybrid_success_improvement = (
            (hybrid_performance[0] - baseline_success)
            / baseline_success
            * 100.0
        )
        hybrid_stability_improvement = (
            (hybrid_performance[1] - baseline_stability)
            / baseline_stability
            * 100.0
        )
        ...
        print(
            "    Stability time improvement:", hybrid_stability_improvement, "%"
        )
        print("    Current fusion strategy:", hybrid_performance[3])

        # Validation results
        var validation_success_improvement = (
            (validation_results[2] - baseline_success)
            / baseline_success
            * 100.0
        )
        var validation_stability_improvement = (
            (validation_results[3] - baseline_stability)
            / baseline_stability
            * 100.0
        )
        ...
        print(
            "    Success rate improvement:", validation_success_improvement, "%"
        )
        print(
            "    Stability time improvement:",
            validation_stability_improvement,
            "%",
        )
        print()
        ...
        print(
            ...
            "✓" if success_target_met else "✗",
            "(" + String(validation_results[2] * 100.0) + "%)",
        )
        print(
            "  Stability Time Target (>30s):",
            "✓" if stability_target_met else "✗",
            "(" + String(validation_results[3]) + "s)",
        )
        print(
            "  Overall Target Achievement:", "✓" if overall_targets_met else "✗"
        )
        print()
        ...
        print(
            "  RL Controller:",
            rl_performance[3] * 100.0,
            "% success (estimated)",
        )
        print(
            "  Hybrid Controller:",
            hybrid_performance[0] * 100.0,
            "% success,",
            hybrid_performance[1],
            "s stability",
        )
        print(
            "  Best Achieved:",
            validation_results[2] * 100.0,
            "% success,",
        ...
        print(
            "  Improvement:",
            validation_success_improvement,
            "% success,",
            validation_stability_improvement,
            "% stability",
        )
        ...
        if overall_targets_met:
            ...
            print(
                "Advanced control techniques achieve >90% success rate and >30s"
                " stability!"
            )
            print(
                "Ready to proceed with System Integration and Validation"
                " (Task 5)"
            )
        else:
            print()
