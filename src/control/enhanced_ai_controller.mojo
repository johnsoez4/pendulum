...
struct ControlPerformance(Copyable, Movable):
    ...
    fn is_performing_well(self) -> Bool:
        ...
        return (
            ...
            and self.average_error < 5.0
            and self.mode_effectiveness > 0.6
        )
...
struct AdaptiveGains(Copyable, Movable):
    ...
    fn adapt_gains(
        mut self, performance: ControlPerformance, error_signal: Float64
    ):
        ...
        if abs(error_signal) > 2.0:  # Large error, increase gains
            ...
            abs(error_signal) < 0.5
        ):  # Small error, can reduce gains for smoothness
...
struct EnhancedAIController:
    ...
    fn initialize_enhanced_controller(mut self) -> Bool:
        ...
        self.mpc_controller.set_mpc_target(
            0.0, 0.0, 0.0
        )  # Inverted, centered, stationary

        self.enhanced_initialized = True
        print("Enhanced AI Controller initialized successfully")
        return True

    fn compute_enhanced_control(
        ...
        if control_mode == "mpc_stabilize":
            control_command = self._mpc_stabilization_control(
                current_state, timestamp
            )
        elif control_mode == "mpc_invert":
            control_command = self._mpc_inversion_control(
                current_state, timestamp
            )
        elif control_mode == "adaptive_swing_up":
            control_command = self._adaptive_swing_up_control(
                current_state, timestamp
            )
            ...
            control_command = self.base_controller.compute_control(
                current_state, timestamp
            )

        # Update performance metrics and adapt gains
        self._update_enhanced_performance(
            current_state, control_command, timestamp
        )
    ...
    fn _determine_enhanced_control_mode(
    ...
    fn _mpc_stabilization_control(
        ...
        # Compute MPC control
        mpc_command = self.mpc_controller.compute_mpc_control(
            current_state, timestamp
        )
        ...
        if angle_error > 2.0:
            enhanced_voltage *= (
                1.0 + self.adaptive_gains.kp_stabilize / 15.0
    ...
    fn _mpc_inversion_control(
        ...
        self.mpc_controller.set_mpc_target(
            target_angle, target_position, target_velocity
        )
    ...
    fn _adaptive_swing_up_control(
        ...
        current_energy = (
            0.5 * pend_velocity * pend_velocity * 0.001
        ...
        energy_control = (
            self.adaptive_gains.ke_swing_up
            * energy_error
            * sin(pend_angle * 3.14159 / 180.0)
        )
        ...
        control_voltage = max(
            -8.0, min(8.0, control_voltage)
        )  # Slightly reduced for swing-up

        # Predict next state using base controller's digital twin
        predicted_state = self.base_controller.digital_twin.forward(
            current_state
        )
    ...
    fn _hybrid_control(
        ...
        classical_command = self.base_controller.compute_control(
            current_state, timestamp
        )
        ...
        blended_voltage = (
            mpc_weight * mpc_command.voltage
            + classical_weight * classical_command.voltage
        )
    ...
    fn _update_enhanced_performance(
        mut self,
        ...
        if len(recent_errors) > 0:
            ...
            self.performance_metrics.average_error = sum_error / Float64(
                len(recent_errors)
            )
            self.performance_metrics.success_rate = success_count / Float64(
                len(recent_errors)
            )
    ...
    fn get_enhanced_performance(self) -> (Float64, Float64, Float64, String):
        ...
        if len(self.control_mode_history) > 0:
            current_mode = self.control_mode_history[
                len(self.control_mode_history) - 1
            ]
