...
struct ControllerConfidence(Copyable, Movable):
    ...
    fn get_best_controller(self) -> String:
        ...
        var max_conf = max(
            self.mpc_confidence,
            max(self.rl_confidence, self.adaptive_confidence),
        )
    ...
    fn is_high_confidence(self) -> Bool:
        ...
        return (
            self.mpc_confidence > CONFIDENCE_THRESHOLD
            or self.rl_confidence > CONFIDENCE_THRESHOLD
            or self.adaptive_confidence > CONFIDENCE_THRESHOLD
        )
...
struct FusionStrategy(Copyable, Movable):
    ...
    fn is_applicable(
        self, performance: ControlPerformance, stability_time: Float64
    ) -> Bool:
        """Check if strategy is applicable for current conditions."""
        return (
            performance.success_rate >= self.performance_threshold
            and stability_time >= self.stability_requirement
        )
...
struct AdvancedHybridController:
    ...
    fn compute_hybrid_control(
        ...
        enhanced_command = self.enhanced_controller.compute_enhanced_control(
            current_state, timestamp
        )
        rl_command = self.rl_controller.compute_rl_control(
            current_state, timestamp
        )

        # Update controller confidence based on recent performance
        self._update_controller_confidence(
            current_state, enhanced_command, rl_command
        )
        ...
        fused_command = self._fuse_control_commands(
            enhanced_command, rl_command, strategy, timestamp
        )

        # Update performance tracking
        self._update_performance_tracking(
            current_state, fused_command, timestamp
        )
    ...
    fn _select_fusion_strategy(
        self, current_state: List[Float64]
    ) -> FusionStrategy:
        """Select optimal fusion strategy based on current conditions."""
        _ = current_state[2]  # pend_angle not used in this function
    ...
    fn _fuse_control_commands(
        self,
        enhanced_cmd: ControlCommand,
        rl_cmd: ControlCommand,
        ...
        adaptive_voltage = (
            enhanced_voltage * 0.8
        )  # Simplified adaptive component

        # Fuse voltages using strategy weights
        var fused_voltage = (
            strategy.mpc_weight * enhanced_voltage
        ...
        if (
            self.controller_confidence.rl_confidence
            > self.controller_confidence.mpc_confidence
        ):
    ...
    fn _update_controller_confidence(
        mut self,
        ...
        self.controller_confidence.rl_confidence = min(
            0.9, rl_performance[3] + 0.1
    ...
    fn _score_strategy(
        self, strategy: FusionStrategy, current_state: List[Float64]
    ) -> Float64:
        ...
        if (
            abs_angle > 30.0
            and abs_angle < 120.0
            and strategy.strategy_name == "rl_dominant"
        ):
            score += 0.3
    ...
    fn _update_performance_tracking(
        mut self,
        current_state: List[Float64],
    ...
    fn get_hybrid_performance(
        self,
    ) -> (Float64, Float64, Float64, String, Bool):
        """
        Get comprehensive hybrid controller performance.

        Returns:
            Tuple containing (success_rate, stability_time, fusion_confidence, current_strategy, meets_targets).
        """
        ...
        if len(self.performance_history) > 0:
            ...
            success_rate = total_success / Float64(
        ...
        meets_targets = (
            success_rate >= SUCCESS_TARGET and avg_stability >= STABILITY_TARGET
        )
