...
struct SafetyMonitor:
    ...
    fn __init__(out self):
        ...
        self.last_command = ControlCommand(
            0.0, 0.0, "init", False, predicted_state
        )
    ...
    fn _check_position_constraints(
    ...
    fn _check_velocity_constraints(
        mut self, current_state: List[Float64], timestamp: Float64
    ):
    ...
    fn _check_acceleration_constraints(
        mut self, current_state: List[Float64], timestamp: Float64
    ):
    ...
    fn _update_safety_status(mut self, timestamp: Float64):
        ...
        if (
            self.safety_status.warnings_active > 0
            or self.safety_status.critical_violations > 0
        ):
        ...
        if (
            self.safety_status.critical_violations == 0
            and not self.safety_status.emergency_stop_active
        ):
            self.safety_status.system_safe = True
        else:
            self.safety_status.system_safe = False
    ...
    fn get_safety_report(self) -> (Int, Int, Float64, Bool):
        ...
        if current_time == 0.0:
            current_time = (
                self.monitoring_start_time + 1.0
            )  # Avoid division by zero

        total_time = current_time - self.monitoring_start_time
        var violation_time = (
            Float64(critical_count) * 0.1
        )  # Estimate violation duration
        uptime_percentage = max(
            0.0, (total_time - violation_time) / total_time * 100.0
        )
        ...
        return (
            ...
            critical_count,
            uptime_percentage,
            self.safety_status.system_safe,
        )
