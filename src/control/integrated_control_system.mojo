...
struct SystemStatus(Copyable, Movable):
    ...
    fn get_success_rate(self) -> Float64:
        ...
        if self.total_control_cycles == 0:
            return 0.0
        return (
            Float64(self.successful_inversions)
            / Float64(self.total_control_cycles)
            * 100.0
        )
...
struct IntegratedControlSystem:
    ...
    fn execute_control_cycle(
        mut self, raw_sensor_data: List[Float64], timestamp: Float64
    ) -> ControlCommand:
        """
        ...
        filtered_state = self.state_estimator.estimate_state(
        ...
        control_input.append(
            raw_sensor_data[3] if len(raw_sensor_data) > 3 else 0.0
        )

        # Step 2: AI Control Computation
        control_command = self.ai_controller.compute_control(
            control_input, timestamp
        )

        # Step 3: Safety Monitoring
        var safety_status = self.safety_monitor.check_safety(
            control_input, control_command
        )

        # Step 4: Apply Safety Override if Necessary
        var final_command = control_command
        if not safety_status.system_safe:
            final_command = self.safety_monitor.apply_safety_override(
                control_command
            )
        ...
        self._update_performance_metrics(
            filtered_state, final_command, timestamp
        )
    ...
    fn _update_performance_metrics(
        ...
        is_inverted = (
            abs(state.pend_angle) < 10.0
        )  # Within 10 degrees of inverted

        if is_inverted:
            self.system_status.current_inversion_time += (
                timestamp - self.last_control_time
            )
    ...
    fn _update_system_status(
        mut self, state: FilteredState, timestamp: Float64
    ):
