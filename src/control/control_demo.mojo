...
from control.integrated_control_system import IntegratedControlSystem, SystemStatus
from control.ai_controller import ControlCommand
from control.safety_monitor import SafetyStatus
from control.state_estimator import FilteredState
...
struct ControlDemo:
    ...
    fn _test_stabilization_control(mut control_system: IntegratedControlSystem):
        ...
        successful_cycles = 0
        total_cycles = 25  # 1 second at 25 Hz
        
        for i in range(total_cycles):
            timestamp = start_time + Float64(i) * 0.04  # 40ms intervals
            
            # Execute control cycle
            command = control_system.execute_control_cycle(current_state, timestamp)
        ...
        stabilization_rate = Float64(successful_cycles) / Float64(total_cycles) * 100.0
    ...
    fn _test_swing_up_control(mut control_system: IntegratedControlSystem):
        ...
        max_angle_achieved = 180.0
        swing_cycles = 50  # 2 seconds at 25 Hz
        
        for i in range(swing_cycles):
            timestamp = 10.0 + Float64(i) * 0.04
        ...
        swing_progress = (180.0 - abs(max_angle_achieved)) / 180.0 * 100.0
    ...
    fn _test_safety_monitoring(mut control_system: IntegratedControlSystem):
        ...
        normal_command = control_system.execute_control_cycle(normal_state, 20.2)
    ...
    fn _test_state_estimation(mut control_system: IntegratedControlSystem):
        ...
        for i in range(noisy_cycles):
            timestamp = 30.0 + Float64(i) * 0.04
            
            # Add simulated noise
            noisy_state = List[Float64]()
            ...
            command = control_system.execute_control_cycle(noisy_state, timestamp)
            
            # Check if control remains stable despite noise
            if abs(command.voltage) < 5.0:  # Reasonable control effort
                estimation_quality += 1.0
        
        filtering_effectiveness = estimation_quality / Float64(noisy_cycles) * 100.0
    ...
    fn _simulate_pendulum_response(current_state: List[Float64], command: ControlCommand) -> List[Float64]:
        ...
        control_force = control_voltage * 0.1  # Voltage to force conversion
        gravity_effect = sin(pend_angle * 3.14159 / 180.0) * 50.0  # Gravity torque
        damping = pend_vel * 0.05  # Velocity damping
    ...
    fn _show_performance_summary(control_system: IntegratedControlSystem):
        ...
        performance = control_system.get_system_performance()
        subsystem_status = control_system.get_subsystem_status()
