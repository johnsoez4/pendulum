...
from src.control.safety_monitor import SafetyStatus
from src.control.state_estimator import FilteredState
...
struct ControlDemo:
    ...
    fn _test_stabilization_control(mut control_system: IntegratedControlSystem):
        ...
        for i in range(total_cycles):
            ...
            command = control_system.execute_control_cycle(
                current_state, timestamp
            )

            # Simulate system response (simplified)
            current_state = ControlDemo._simulate_pendulum_response(
        ...
        stabilization_rate = (
            Float64(successful_cycles) / Float64(total_cycles) * 100.0
        )
    ...
    fn _test_safety_monitoring(mut control_system: IntegratedControlSystem):
        ...
        normal_command = control_system.execute_control_cycle(
            normal_state, 20.2
        )
    ...
    fn _test_state_estimation(mut control_system: IntegratedControlSystem):
        ...
        for i in range(noisy_cycles):
            ...
            noisy_state.append(
                clean_state[0] + (Float64(i % 3) - 1.0) * 0.1
            )  # Position noise
            noisy_state.append(
                clean_state[1] + (Float64(i % 5) - 2.0) * 5.0
            )  # Velocity noise
            noisy_state.append(
                clean_state[2] + (Float64(i % 4) - 1.5) * 2.0
            )  # Angle noise
        ...
        filtering_effectiveness = (
            estimation_quality / Float64(noisy_cycles) * 100.0
        )
    ...
    @staticmethod
    fn _simulate_pendulum_response(
        ...
        gravity_effect = (
            sin(pend_angle * 3.14159 / 180.0) * 50.0
        )  # Gravity torque
        damping = pend_vel * 0.05  # Velocity damping

        # Update pendulum velocity (simplified)
        var new_pend_vel = (
            pend_vel + (gravity_effect - damping + control_force) * dt
        )
