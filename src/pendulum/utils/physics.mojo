...
alias SAFETY_ACTUATOR_MARGIN = 0.2  # inches - safety margin for actuator
alias SAFETY_VOLTAGE_MARGIN = 0.5  # volts - safety margin for voltage
...
struct PendulumPhysics:
    ...
    fn integrate_step(self, state: PendulumState, dt: Float64) -> PendulumState:
        ...
        # Final integration
        new_cart_pos = state.cart_position + (dt / 6.0) * (
            k1[1] + 2.0 * k2[1] + 2.0 * k3[1] + k4[1]
        )
        new_cart_vel = state.cart_velocity + (dt / 6.0) * (
            k1[0] + 2.0 * k2[0] + 2.0 * k3[0] + k4[0]
        )
        new_pend_angle = state.pendulum_angle + (dt / 6.0) * (
            k1[3] + 2.0 * k2[3] + 2.0 * k3[3] + k4[3]
        )
        new_pend_vel = state.pendulum_velocity + (dt / 6.0) * (
