...
struct GPUNeuralLayer:
    ...
    fn __init__(out self, input_size: Int, output_size: Int, activation: String = "tanh", use_gpu: Bool = True):
...
struct GPUPendulumNeuralNetwork:
    ...
    var layers: List[GPUNeuralLayer]
    ...
    fn __init__(out self, use_gpu: Bool = True):
        ...
        self.layers = List[GPUNeuralLayer]()
    ...
    fn _build_architecture(mut self):
        """Build the GPU-accelerated neural network architecture."""
        # Input layer to first hidden layer
        var layer1 = GPUNeuralLayer(MODEL_INPUT_DIM, MODEL_HIDDEN_SIZE, "tanh", self.use_gpu)
        self.layers.append(layer1)
        
        # Hidden layers
        for _ in range(MODEL_HIDDEN_LAYERS - 1):
            var hidden_layer = GPUNeuralLayer(MODEL_HIDDEN_SIZE, MODEL_HIDDEN_SIZE, "tanh", self.use_gpu)
            self.layers.append(hidden_layer)
        
        # Output layer (linear activation for regression)
        var output_layer = GPUNeuralLayer(MODEL_HIDDEN_SIZE, MODEL_OUTPUT_DIM, "linear", self.use_gpu)
